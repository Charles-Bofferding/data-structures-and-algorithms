...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  return starWarsArr.sort((a,b) => {
    let ah = parseInt(a.height);
    let bh = parseInt(b.height);
    if(ah > bh){
      return -1;
    } else if (ah < bh){
      return 1;
...
//I made this using both split and splice before getting to the next problem
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  for(let itemStr of recipe.ingredients){
    let newArr = itemStr.split(' ');
...
//So I think my previous answer also satisfies this problem statement
