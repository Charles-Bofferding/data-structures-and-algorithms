...
const addValues = (arr) => {
  // Solution code here...
  const theReducer = (sum, value) => sum + value;
  return arr.reduce(theReducer,0);
};
...
const addPurchases = (arr) => {
  // Solution code here...
  const theReducer = (sum, value) => sum + value.purchasePrice;
  return arr.reduce(theReducer,0);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const theReducer = (sum, value) => sum + 1;
  return arr.reduce(theReducer,0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  let result = [];
  let theReducer = (accumulator, currentValue, index, arr) => {
    result.push(currentValue.name);
  };
  arr.reduce(theReducer, result);
  return result;
};
...
const reversedString = (str) => {
  ...
  //build array from string so we can use reducer
  let arr = [];
  for(let i = 0; i < str.length; i++){
    arr.push(str.charAt(i));
  }

  //use reduce to flip those values
  const theReducer = (sum, value) => value + sum;
  return arr.reduce(theReducer,'');
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce(((sum,value) => {sum + value.children.length}),0);
};
...
const calculateAverage = (arr) => {
  ...
  let size = arr.length
  const theReducer = (sum, value) => value + sum;
  let total = arr.reduce(theReducer,0);
  return total / size;
};
...
const extractChildren = (arr) => {
  ...
  //filter the array
  //reduce to pull all the children names
};
