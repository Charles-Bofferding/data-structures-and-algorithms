...
function transformToLis(obj){
  ...
  for (let pair of data){
    results.push('<li>' + pair[0] + ': ' + pair[1] + '</li>');
  }
  return results;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for(let arr of input){
    temp = arr.filter( x => {
      return ((!(x % 5)) && (typeof x == 'number'));
    });
    let temp2 = temp.map(x => (Math.pow(2, x)));
    results.push(temp2);
  }
...
let findShortest = (data) => {
  // Solution code here...
  let shorty = {height: '300'};
  for(let person of data){
    if(parseInt(person.height) < parseInt(shorty.height)){
      shorty = person;
    }
  }
  return shorty.name;
