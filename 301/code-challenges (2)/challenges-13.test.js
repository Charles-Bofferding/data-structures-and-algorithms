...
const standardizePhoneNumbers = (arr) => {
  ...
  let results = [];
  for(let num of arr){
    let start = num.slice(1,4);
    let mid = num.slice(6,9);
    let end = num.slice(10,14);
    results.push(start+mid+end)
  }
  return results;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let filtered = arr.filter(str => /:\)/.test(str));
