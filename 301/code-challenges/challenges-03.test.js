...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  //for each bad value
  //create temp array and fill with filter of first array wihtout bad value
  //set temp array to regular and go again

  for(let bad of forbiddenValues){
    arr = arr.filter(item => !(item == bad));
  }
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter(aStat => (aStat.baseStat > minBaseStat));
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let newArr = arr.filter(aStat => (aStat.baseStat > minBaseStat));
  let solution = [];
  for(let aStat of newArr){
    solution.push(aStat.stat.name);
  }
  return solution;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let justNums = arr.filter(item => Number.isInteger(item));
  return justNums.map(num => {
      if(num % 2){
        return 'odd';
      }else{
        return 'even';
